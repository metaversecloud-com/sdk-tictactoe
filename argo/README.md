...
**Secrets:** no plaintext committed. `tictactoe0-sealedsecret.yaml` holds only ciphertext
(sealed strict-scope for namespace sdk-apps-dev); the controller unseals it into
`Secret tictactoe0-secrets`, consumed via `envFrom`. Non-secret env (incl. public
`INTERACTIVE_KEY`) lives in `tictactoe0-config`.

## Environment

| Env | Service | Namespace | Host | Health |
| --- | ------- | --------- | ---- | ------ |
| `dev` | `tictactoe0` | `sdk-apps-dev` | tictactoe0-dev-topia.topia-rtsdk.com | `/api/system/health` |
