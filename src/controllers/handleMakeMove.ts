...
export const handleMakeMove = async (req: Request, res: Response) => {
  try {
    console.log("🚀 ~ file: handleMakeMove.ts:9 ~ query:", req.query);
    console.log("🚀 ~ file: handleMakeMove.ts:9 ~ body:", req.body);
    ...
    const droppedAsset = await DroppedAsset.drop(webImageAsset, {
      // @ts-ignore
      layer0: "",
      layer1: `${process.env.API_URL}/${visitorId === game.player1?.visitorId ? "pink_cross" : "blue_o"}.png`,
    ...
    console.log("🚀 ~ file: handleMakeMove.ts:71 ~ winningCombo:", winningCombo);
    ...
    console.log("🚀 ~ file: handleMakeMove.ts:77 ~ finishLineOptions:", finishLineOptions);
